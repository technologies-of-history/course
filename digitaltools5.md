...
1. First, we'll want to combine all our csv files into one giant spreadsheet, but as we combine them, we'll also want to add in the gender information that will be necessary for our network analysis. So, let's open **women_to_women.csv** first. Delete the first **Description** column as well as the **Location** column, then add a column after both **Author** and **Recipient** labeled **Gender**. Since this is the spreadsheet of women writing to women, we know that all individuals are women, so we can copy-and-paste **Female** next to all entries in each column.
