...
- `layout: post` tells the Jekyll develope to use the layout for a blog post for the Markdown that follows. Other pages in the `docs` folder are formatted as `page`, which tells the developer to use a different layout for the Markdown that follows.
- `title: Your title here` is pretty self explanatory. You should type whatever title you'd like for your post after the colon.
...
4. You'll now see a gray box at the top that shows you the **base** repository you'll be sending your changes to and the **head** repository, which is yours. NOTE: Be sure that yours is the **head** and the main branch is the **base**.
5. Name your pull request **LastName_dt1** and then click **Create pull request**.
